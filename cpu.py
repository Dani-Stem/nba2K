...
class CPU(pygame.sprite.Sprite):
    def __init__(self, pos, groups):
        ...
        self.jump_sound = pygame.mixer.Sound("tipoff/images/sounds/jump.wav")
        self.jump_sound.set_volume(0.03)

        self.landing_sound = pygame.mixer.Sound("tipoff/images/sounds/land.wav")
        self.landing_sound.set_volume(0.02)

    def import_assets(self):
        self.idle_animation = [
            pygame.image.load(
                f"tipoff/images/lebron/lebron_idle_left/{frame}.png"
            ).convert_alpha()
            for frame in range(10)
        ]
        self.jump_animation = [
            pygame.image.load(
                f"tipoff/images/lebron/lebron_jump_left/{frame}.png"
            ).convert_alpha()
            for frame in range(9)
        ]
        self.land_animation = [
            pygame.image.load(
                f"tipoff/images/lebron/lebron_land_left/{frame}.png"
            ).convert_alpha()
            for frame in range(9)
        ]
